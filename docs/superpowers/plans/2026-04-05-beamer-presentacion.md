# Beamer Presentación — Replicación SVAR México

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Crear `presentacion_replicacion.tex` — presentación Beamer compilable (~15 slides) en español con tema Metropolis para seminario interno sobre la replicación de Carrillo & Elizondo (2015/2018).

**Architecture:** Archivo LaTeX único que se construye en dos tareas: (1) escribir el `.tex` completo con todas las secciones, código MATLAB embebido y tablas de comparación; (2) verificar que compila sin errores. Las imágenes se referencian desde `figures/` relativo al directorio raíz del proyecto donde vivirá el `.tex`.

**Tech Stack:** LaTeX/Beamer, tema Metropolis, paquetes `listings` (chunks MATLAB), `booktabs`, `amsmath`, `graphicx`, `babel` (español).
...
| Archivo | Acción |
|---|---|
| `presentacion_replicacion.tex` | CREAR — presentación completa |

---

### Task 1: Escribir el archivo LaTeX completo

**Files:**
- Create: `presentacion_replicacion.tex`

El archivo vive en la raíz del proyecto:
`C:\Users\carlo\OneDrive - INSTITUTO TECNOLOGICO AUTONOMO DE MEXICO\Escritorio\varras\presentacion_replicacion.tex`

Las imágenes se referencian como `figures/nombre.png` (ruta relativa al `.tex`).

- [ ] **Step 1: Crear el archivo con el contenido completo**

Escribir exactamente el siguiente contenido en `presentacion_replicacion.tex`:
...
\begin{tabular}{lll}
  ...
  $\hat{\beta}_{lower}$ & $-0.7$                          & $-0.715$                               \\
  \bottomrule
\end{tabular}
\end{center}
\vspace{0.3cm}
\small
El ajuste estacional se aplica a inflaci\'on subyacente, INPP y M2 \emph{antes}
de calcular los gaps con filtro HP.
\end{frame}

% ── SLIDE 4: SERIES ORIGINALES ───────────────────────────────────────────────
\begin{frame}{\S1 Series originales con tendencia HP ($\lambda=129{,}600$)}
\begin{center}
  \includegraphics[width=0.95\textwidth]{figures/fig_series_originales.png}
\end{center}
\end{frame}
...
\begin{center}
  \includegraphics[width=0.95\textwidth]{figures/fig_series_gaps.png}
\end{center}
\end{frame}
...
\column{0.55\textwidth}
  \includegraphics[width=\textwidth]{figures/tabla_adf_pp.png}
  \vspace{0.15cm}
  \includegraphics[width=\textwidth]{figures/tabla_varsoc.png}
\column{0.45\textwidth}
  \textbf{Estabilidad del VAR($p=3$):}
  \vspace{0.2cm}
  \includegraphics[width=\textwidth]{figures/varstable.png}
  \vspace{0.2cm}
  \small
  \begin{itemize}
...
\begin{columns}[T]
\column{0.55\textwidth}
  \includegraphics[width=\textwidth]{figures/tabla_lm_norm.png}
\column{0.45\textwidth}
...
\column{0.65\textwidth}
  \includegraphics[width=\textwidth]{figures/fig7_exclusion.png}
\column{0.35\textwidth}
  ...
  \vspace{0.3cm}
  \small
  Por construcci\'on del ordenamiento Cholesky, las variables 1--4
  no reaccionan en $h=0$.
\end{columns}
\end{frame}

% ── SLIDE 9: BOOTSTRAP RUNKLE (CÓDIGO) ───────────────────────────────────────
...
\begin{columns}[T]
\column{0.65\textwidth}
  \includegraphics[width=\textwidth]{figures/fig8_sign_restrictions.png}
\column{0.35\textwidth}
  \small
  ...
  \vspace{0.3cm}
  \small
  Modelos aceptados: \textbf{198,654} (est\'andar) \\
  Modelos aceptados: \textbf{66,874} (aumentado) \\[4pt]
...
\begin{enumerate}\small
  ...
  \end{enumerate}
  \item Reportar mediana e $\mathrm{IC}_{68\%}=[p_{16},\,p_{84}]$ sobre IRFs aceptadas
\end{enumerate}
\vspace{0.15cm}
\small
\alert{La restricci\'on aumentada impone consistencia con la elasticidad estimada
$\hat{\beta}^-=-0.400$, filtrando modelos con respuesta de inflaci\'on demasiado d\'ebil.}
\end{frame}
...
\column{0.60\textwidth}
  \includegraphics[width=\textwidth]{figures/fig9_comparison.png}
\column{0.40\textwidth}
  ...
  \vspace{0.2cm}
  \small
  \textcolor{green!60!black}{\textbf{Price puzzle eliminado}} con
  restricciones de signo en las tres estrategias. \checkmark
\end{columns}
\end{frame}
...
\begin{tabular}{p{2.8cm}p{2.8cm}p{2.8cm}p{3.5cm}}
  \toprule
...
Confirmar que `presentacion_replicacion.tex` existe en la raíz del proyecto y tiene contenido (>100 líneas).

---

### Task 2: Verificar compilación LaTeX

**Files:**
- Read: `presentacion_replicacion.tex`

- [ ] **Step 1: Compilar con pdflatex (primera pasada)**

Desde el directorio raíz del proyecto:
```bash
cd "C:/Users/carlo/OneDrive - INSTITUTO TECNOLOGICO AUTONOMO DE MEXICO/Escritorio/varras"
pdflatex -interaction=nonstopmode presentacion_replicacion.tex
```

Expected: termina sin `! Fatal error` ni `! LaTeX Error`. Pueden aparecer warnings de `babel` o `metropolis` — son normales.

- [ ] **Step 2: Segunda pasada (resolver referencias cruzadas)**

```bash
pdflatex -interaction=nonstopmode presentacion_replicacion.tex
```

Expected: `Output written on presentacion_replicacion.pdf` en la última línea del log.

- [ ] **Step 3: Verificar PDF generado**

```bash
ls -lh presentacion_replicacion.pdf
```

Expected: archivo de >50 KB existe.

- [ ] **Step 4: Revisar el log en busca de errores reales**

```bash
grep -i "^!" presentacion_replicacion.log | head -20
```

Expected: sin líneas que comiencen con `!`.

Si aparece un error de paquete faltante (p. ej. `metropolis.sty not found`), instalar con:
```bash
tlmgr install beamertheme-metropolis
tlmgr install pgfopts
```

Si `pdflatex` no está en el PATH, usar la ruta completa de la distribución TeX instalada (MiKTeX en Windows: `C:/Program Files/MiKTeX/miktex/bin/x64/pdflatex.exe`).
